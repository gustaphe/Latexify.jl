...
@test latexify(rn; env=:chem) == raw"\begin{align}
                                 \require{mhchem}
...
@test latexify(rn; env=:chem, expand=false) == raw"\begin{align}
                                               \require{mhchem}
...
@test md(rn; env=:chem) == raw"\begin{align}
                           \require{mhchem}
                           \ce{ \varnothing &->[\frac{v_{x} \cdot y^{2}}{k_{x}^{2} + y^{2}}] x}\\
                           \ce{ \varnothing &->[p_{y}] y}\\
                           \ce{ x &->[d_{x}] \varnothing}\\
...
@test md(ode; env=:chem) == raw"\begin{align}
                            \require{mhchem}
