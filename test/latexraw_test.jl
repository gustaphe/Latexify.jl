...
@test latexify(:((-1)^2)) == replace(raw"$\left( -1 \right)^{2}$", "\r\n" => "\n")
@test latexify(:($(1 + 2im)^2)) ==
    replace(raw"$\left( 1+2\mathit{i} \right)^{2}$", "\r\n" => "\n")
@test latexify(:($(3//2)^2)) ==
    replace(raw"$\left( \frac{3}{2} \right)^{2}$", "\r\n" => "\n")
...
@test latexraw("1 - 2 - (- 3 -(2 - 8) + 4)") ==
    raw"1 - 2 - \left(  - 3 - \left( 2 - 8 \right) + 4 \right)"
@test latexraw(:(-$(3 + 5im))) == raw" - \left( 3+5\mathit{i} \right)"
@test latexraw(:($(3 + 4im) - $(3 + 5im))) ==
    raw"3+4\mathit{i} - \left( 3+5\mathit{i} \right)"
...
@test latexify(
    [32894823 1.232212 :P_1; :(x / y) 1.0e10 1289.1]; env=:equation, fmt="%.2e"
) == replace(
    raw"""\begin{equation}
    \left[
    \begin{array}{ccc}
...
@test latexify(
    [32894823 1.232212 :P_1; :(x / y) 1.0e10 1289.1]; env=:mdtable, fmt="%.2e"
) == Markdown.md"""|    $3.29e+07$ | $1.23e+00$ |    $P_{1}$ |
     | -------------:| ----------:| ----------:|
     | $\frac{x}{y}$ | $1.00e+10$ | $1.29e+03$ |
     """

@test latexify(1234.2234; env=:inline, fmt="%.2e") == raw"$1.23e+03$"

@test latexify(1234.2234; env=:raw, fmt="%.2e") == raw"1.23e+03"
...
@test latexify(["3*$(func)(x)^2/4 -1" for func in test_functions]) == replace(
    raw"""\begin{equation}
    \left[
    \begin{array}{c}
