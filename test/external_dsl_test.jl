...
@test latexify(f; env=:align, cdot=false) == raw"\begin{align}
                                             \frac{dx}{dt} =& y c_{1} - x \\
                                             \frac{dy}{dt} =& x^{c_{2}} - y
                                             \end{align}
                                             "

@test latexify(f; env=:align, cdot=true) == raw"\begin{align}
...
@test latexify(rn; cdot=false) == raw"\begin{align}
...
@test latexify(rn; cdot=true) == raw"\begin{align}
...
@test latexify(rn; env=:chemical, cdot=false) == raw"\begin{align}
                                                 \require{mhchem}
...
@test latexify(rn; env=:chemical, cdot=true) == raw"\begin{align}
                                                \require{mhchem}
...
# tabular
@test latexify(rn.symjac; env=:tabular, cdot=false) == raw"\begin{tabular}{ccccccc}
...
@test latexify(rn.symjac; env=:tabular, cdot=true) == raw"\begin{tabular}{ccccccc}
...
@test latexraw(f) ==
    ["\\frac{dx}{dt} = \\frac{y}{c_{1}} - x", "\\frac{dy}{dt} = x^{c_{2}} - y"]
...
@test latexify(f) == raw"\begin{align}
                     \frac{dx}{dt} =& \frac{y}{c_{1}} - x \\
                     \frac{dy}{dt} =& x^{c_{2}} - y
                     \end{align}
                     "

@test latexify(f; rows=1) == raw"\begin{align}
                             \frac{dx}{dt} =& \frac{y}{c_{1}} - x
                             \end{align}
                             "

@test latexify(f; env=:align, field=:symjac) == raw"\begin{align}
                                                \frac{dx}{dt} =& -1 \cdot 1 =& c_{1}^{-1} \\
                                                \frac{dy}{dt} =& x^{-1 + c_{2}} \cdot c_{2} =& -1 \cdot 1
                                                \end{align}
                                                "

@test latexalign(["d$(x)/dt" for x in f.syms], f.funcs) == raw"\begin{align}
                                                           \frac{dx}{dt} =& \frac{y}{c_{1}} - x \\
                                                           \frac{dy}{dt} =& x^{c_{2}} - y
                                                           \end{align}
                                                           "
...
@test latexalign(rn; clean=true).s == raw"\begin{align}
